...
async function check(un) {
    try {
        const result = await (0, package_json_1.default)(un.options.pkg.name, { fullMetadata: true, allVersions: true });
...
exports.check = check;
async function getChangelogURL(changeLogBaseUrl, distTag) {
    if (changeLogBaseUrl) {
        try {
